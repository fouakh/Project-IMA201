...
def display_alpha_beta_scatter(lab_image):
    # Extract the α and β channels
    alpha = lab_image[:, :, 1].flatten()  # α channel
    beta = lab_image[:, :, 2].flatten()   # β channel

    # Create a scatter plot of α vs β
    plt.figure(figsize=(10, 6))
    plt.scatter(alpha, beta, s=1, color='blue', alpha=0.5)
    plt.title('αβ Diagram')
    plt.xlabel('α (green-red component)')
    plt.ylabel('β (blue-yellow component)')
    plt.xlim(-1, 1)  # Adjust limits according to expected values
    plt.ylim(-1, 1)  # Adjust limits according to expected values
    plt.grid()
    plt.show()